/**
 * @file const/index.ts
 * @description 应用所有核心常量、类型、枚举和数据结构定义，统一管理事件、分类、设置、日历、农历等相关数据。
 * 
 * 为什么这样做：
 * - 通过枚举和类统一事件类型和结构，保证前后端数据一致性和类型安全。
 * - 所有类型定义（事件、分类、设置、日历等）集中管理，便于扩展和维护，提升团队协作效率。
 * - 预设颜色选项、日历视图、农历数据等常量统一管理，减少硬编码和重复定义，提升代码可读性和灵活性。
 * - 农历相关数据和名称集中定义，便于日历组件直接调用，支持本地化和多语言扩展。
 * - 导出所有核心类型和常量，方便各业务模块直接引用，提升开发效率和代码规范性。
 */
