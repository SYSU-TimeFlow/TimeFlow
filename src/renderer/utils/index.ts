/**
 * @file utils/index.ts
 * @description 日历应用的通用工具函数集合，负责日期、时间、农历、事件布局、格式化等核心计算和辅助逻辑。
 * 
 * 为什么这样做：
 * - 所有日期和时间相关计算（如周/月视图、农历、跨天事件等）集中管理，提升代码复用性和维护性。
 * - 格式化函数统一输出，保证前后端显示一致性，便于国际化和多端适配。
 * - 农历算法和数据处理独立封装，支持中国用户本地化需求，便于后续扩展。
 * - 事件布局和分组算法优化日历渲染性能，提升复杂场景下的交互体验。
 * - 颜色对比、输入格式化等辅助函数提升界面可读性和用户体验。
 * - 所有核心工具均有注释说明，便于团队协作和后续维护。
 */
