/**
 * @file useLogger.js
 * @description Vue 组件日志组合式 API，提供组件级别的日志采集、性能跟踪和安全错误处理，便于前端行为分析和主进程日志归档。
 * 
 * 为什么这样做：
 * - 通过组合式 API 封装日志方法，便于在各组件内统一采集用户操作、API 调用、性能指标、生命周期等关键行为。
 * - 日志自动带上组件名和上下文信息，提升问题定位和行为分析的准确性。
 * - 提供 startTimer 和 createAsyncTracker，支持性能测量和异步操作跟踪，便于性能优化和异步流程监控。
 * - 错误处理 API（safeAsync/safeSync）统一捕获异常并记录日志，减少遗漏和重复代码，提升健壮性。
 * - 响应式 errorCount/lastError 便于组件内展示错误状态和调试。
