/**
 * @file page.ts
 * @description 日历主页面功能模块，负责管理所有模态框状态、页面交互、命令处理、滚动控制、全局键盘事件和消息弹窗等核心逻辑。
 * 
 * 为什么这样做：
 * - 通过 ref 管理所有模态框和弹窗状态，保证页面响应式和多场景切换，提升用户体验。
 * - 所有模态框操作（打开/关闭/切换）统一封装，减少重复代码，便于维护和扩展。
 * - 事件、待办、分类等业务逻辑与 UI 状态解耦，便于复用和测试。
 * - 支持命令行跳转、滚动控制、全局键盘事件等高级交互，提升效率和可访问性。
 * - 消息弹窗和确认弹窗采用 Promise 封装，支持异步流程和用户反馈，提升交互灵活性。
 * - 通过 storeContext 复用全局状态，便于与其他模块协作和数据同步。
